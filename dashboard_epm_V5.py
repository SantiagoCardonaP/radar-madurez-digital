...
file = st.file_uploader("Sube archivo CSV (delimitado por punto y coma)", type="csv")

if file:
    df = pd.read_csv(file, sep=";")

    # Nuevos filtros incluyendo región
    region = st.multiselect("Región", df['Región'].dropna().unique())
    ...
    df_filtered = df.copy()
    if region:
        df_filtered = df_filtered[df_filtered['Región'].isin(region)]
    if filial:
